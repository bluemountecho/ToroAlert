...
const Beta = ({ updateIndicator }) => {
  ...
  useEffect(() => {
    ...
    if (movesAbove.isChecked) {
      beta.above = movesAbove.value;
      beta.isSelected = true;
      beta.notification = `If BETA is above ${movesAbove.value}`;
    }
    if (movesBelow.isChecked) {
      beta.below = movesBelow.value;
      beta.isSelected = true;
      beta.notification = `If BETA is below ${movesBelow.value}`;
    }
    updateIndicator("beta", beta);
  ...
  return (
    <div className="border-t border-ternary-blue p-5 mt-3 w-[70%]">
      <div className="flex items-center space-x-2">
        ...
        <h3 className="text-xl font-medium text-primary-blue">Beta (BETA)</h3>
      </div>
      {open && (
