...
const RelativeStrengthIndex = ({ updateIndicator }) => {
  ...
  useEffect(() => {
    ...
    if (movesAbove.isChecked) {
      rsi.above = movesAbove.value;
      rsi.isSelected = true;
      rsi.notification = `When RSI moves above ${movesAbove.value}`;
    }
    if (movesBelow.isChecked) {
      rsi.below = movesBelow.value;
      rsi.isSelected = true;
      rsi.notification = `When RSI moves below ${movesBelow.value}`;
    }
