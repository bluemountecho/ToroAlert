...
const AverageDirectionalIndex = ({ updateIndicator }) => {
  ...
  useEffect(() => {
    ...
    if (movesAbove.isChecked) {
      adx.above = movesAbove.value;
      adx.isSelected = true;
      adx.notification = `If ADX moves above ${movesAbove.value}`;
    }
    if (movesBelow.isChecked) {
      adx.below = movesBelow.value;
      adx.isSelected = true;
      adx.notification = `If ADX moves below ${movesBelow.value}`;
    }
    updateIndicator("adx", adx);
