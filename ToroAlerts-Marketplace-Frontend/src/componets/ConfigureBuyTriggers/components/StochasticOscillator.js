...
const StochasticOscillator = ({ updateIndicator }) => {
  ...
  useEffect(() => {
    ...
    if (movesAbove.isChecked) {
      stoch.above = movesAbove.value;
      stoch.isSelected = true;
      stoch.notification = `If STOCH moves above ${movesAbove.value}`;
    }
    if (movesBelow.isChecked) {
      stoch.below = movesBelow.value;
      stoch.isSelected = true;
      stoch.notification = `If STOCH moves below ${movesBelow.value}`;
    }
    updateIndicator("stoch", stoch);
